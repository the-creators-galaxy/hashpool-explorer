...
import type { TokenID } from "@bugbytes/hapi-proto";
import { config } from "@/models/config";
import {
  is_entity_id,
  tokenID_to_keyString,
  type EntityIdKeyString,
} from "@bugbytes/hapi-util";
const props = defineProps<{
  value: TokenID | EntityIdKeyString | undefined;
}>();
const data = computed(() => {
  const keyString = !props.value
    ...
    ? props.value
    : tokenID_to_keyString(props.value);
  if (keyString) {
    const url = `${config.value.hashscanBaseUrl}/token/${keyString}`;
