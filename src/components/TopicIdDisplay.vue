...
import type { TopicID } from "@bugbytes/hapi-proto";
import { config } from "@/models/config";
import {
  is_entity_id,
  topicID_to_keyString,
  type EntityIdKeyString,
} from "@bugbytes/hapi-util";
const props = defineProps<{
  value: TopicID | EntityIdKeyString | undefined;
}>();
const data = computed(() => {
  const keyString = !props.value
    ...
    ? props.value
    : topicID_to_keyString(props.value);
  if (keyString) {
    const url = `${config.value.hashscanBaseUrl}/topic/${keyString}`;
